...
class FJQRImageGenerateUtil: NSObject {
    ...
    //生成高清的UIImage
    static private func setupHighDefinitionUIImage(_ image: CIImage, size: CGFloat) -> UIImage {
    ...
    //生成边框
    static private func circleImageWithImage(_ sourceImage: UIImage, borderWidth: CGFloat, borderColor: UIColor) -> UIImage {
    ...
    static func combineImage(_ image: UIImage, qrImage:UIImage, width: CGFloat, height: CGFloat) -> UIImage{
        //开启图片上下文
        UIGraphicsBeginImageContext(image.size)
        //绘制背景图片
        image.draw(in: CGRect(origin: CGPoint.zero, size: image.size))
        qrImage.draw(in: CGRect(x: 5, y: image.size.height-height-5, width: width, height: height))
        //取出绘制好的图片
        let newImage = UIGraphicsGetImageFromCurrentImageContext()
        //关闭上下文
        UIGraphicsEndImageContext()
